...
\emph{sense}, either {\tt minimize} or {\tt maximize} and an objective 
expression.
...
$\phi(g_1(x),\ldots,
g_n(x); \alpha)$ is convex if the atom $\phi$ is convex and for each $i$
\begin{itemize}
...
affine if $\phi$ is affine and $g_1,\ldots, g_n$ are all affine. This extends
the basic idea that the sum of two convex functions is convex.
