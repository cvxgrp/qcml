...
class Dimension(object):
        ...
        if isinstance(value, Dimension):
            ...
            if isinstance(value, str):
                try:
                    ...
                    # create a Dimension object for it, with the bogus string
                    # as the key
                    self.__data = {value: 1}
    ...
    def __eq__(self,other):
        ...
        if isinstance(other, str):
            return self.__str_helper(False) == other
    ...
    def __str_helper(self, in_dict):
        ...
        if in_dict:
            ...
            # prints coeffs > 1 (ignores 0 coeff)
            terms = ["%s*%s" % (v,python_dimension(k)) for k,v in self.__data.iteritems() if k != '1' and v > 1]
            # prints coeff == 1
            terms.extend(["%s" % (python_dimension(k)) for k,v in self.__data.iteritems() if k != '1' and v == 1])
        # adds constant
