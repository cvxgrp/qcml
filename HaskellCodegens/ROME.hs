module CodeGenerator.ROME(codegenROME) where
  ...
  codegenROME prob = 
    ...
    in unlines $ ["c_ = sparse(" ++ n ++ ",1);",
      ...
      "A__ = [A_; G_]; b__ = [b_; h_];",
	  -- "% scale A__",
	  -- "A__ = diag(1./norms(A__'))*A__; b__ = diag(1./norms(A__'))*b__;",
      "data_.A = A__; data_.b = b__; data_.c = c_;",
      "params_.GEN_PLOTS = false; params_.alpha = 1.0; % alpha is overrelaxation",
      -- "params_.EPS_ABS=1e-6; params_.EPS_REL=1e-6; params_.MAX_ITERS=5000;",
      "[x_codegen, y_, scoop_status] = coneOS_matlab(data_,cone_,params_);",
      "%[x_codegen, y_, scoop_status] = coneOS(A__,b__,c_, cone_);",
      "%{",
      "cvx_begin",
      ...
      "    for i = 1:cone_.k_soc,",
      "      norm(ss(kk+2:kk+cone_.ns_soc(i))) <= ss(kk+1)",
  ...
  coneVar (SOCelem vars) = 
    let n = fromIntegral $ rows (vars!!0)
